...
{
    Cylinder cyl("cylinder");
    REQUIRE(cyl.GetMinimum() == -std::numeric_limits<float>::max());
    REQUIRE(cyl.GetMaximum() == std::numeric_limits<float>::max());
}
...
TEST_CASE("The normal vector on a cylinder's end caps", "[cylinders]")
{
    ...
    const std::vector<TestCase> cases = {
        {"point on lower cap 1", Point(0.f,  1.f, 0.f),  Vector(0.f, -1.f, 0.f)},
